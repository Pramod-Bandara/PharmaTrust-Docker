...
function generateAnomalyReading() {
  ...
  if (anomalyType < 0.4) {
    ...
    return {
      ...
      humidity: Math.random() < 0.5 ? 15 + Math.random() * 10 : 75 + Math.random() * 20, // <25% or >75%
      isAnomaly: true,
      severity: 'medium'
    };
  }
...
async function seedDatabase() {
  ...
  try {
    ...
    delete users.forEach(user => delete user.password); // Remove plain password
    ...
    for (let i = 0; i < MEDICINE_TEMPLATES.length; i++) {
      for (let j = 0; j < 2; j++) {
        ...
        const supplyChain = [];
        for (let k = 0; k <= stageIndex && k < SUPPLY_CHAIN_STAGES.length; k++) {
        ...
        const batch = {
          ...
          blockchainHash: Math.random() < 0.7 ? `0x${crypto.randomBytes(32).toString('hex')}` : null,
          supplyChain,
          qualityStatus,
          expiryDate: new Date(createdDate.getTime() + 2 * 365 * 24 * 60 * 60 * 1000).toISOString(), // 2 years from creation
    ...
    for (const batchId of batchIds) {
      ...
      // Generate readings for the last 7 days (every 30 minutes = 48 readings per day)
      ...
      for (let i = 0; i < totalReadings; i++) {
        ...
        const reading = shouldBeAnomaly ? generateAnomalyReading() : generateNormalReading();
    ...
    console.log(`   ✅ Created ${environmentalData.length} environmental readings`);
    
    // Create indexes for performance
    console.log('📊 Creating database indexes...');
    await db.collection('medicine_batches').createIndex({ batchId: 1 }, { unique: true });
    ...
    await db.collection('environmental_data').createIndex({ batchId: 1, timestamp: -1 });
    ...
    console.log(`   ⚠️  Anomalies: ${environmentalData.filter(r => r.isAnomaly).length}`);
    console.log(`   🔗 Blockchain Records: ${batches.filter(b => b.blockchainHash).length}`);
    
    console.log('\n🔑 Demo Login Credentials:');
    DEMO_USERS.forEach(user => {
      console.log(`   ${user.role.padEnd(12)}: ${user.username} / ${user.password}`);
    });
