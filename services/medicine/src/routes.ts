...
router.get('/batches', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const sort = getSortParams(req.query) as Sort;
...
router.get('/batches/:batchId', requireAuth, async (req: Request, res: Response) => {
  try {
...
// Create new batch
router.post('/batches', requireAuth, requireRole('manufacturer', 'admin'), async (req: Request, res: Response) => {
  try {
    ...
    const initialSupplyChainEntry: SupplyChainEntry = {
      stage: 'manufacturer',
      timestamp: now,
...
// Update batch
router.put('/batches/:batchId', requireAuth, async (req: Request, res: Response) => {
  try {
    const { batchId } = req.params;
    ...
    if (updateData.qualityStatus) updateFields.qualityStatus = updateData.qualityStatus;
...
router.post('/batches/:batchId/transfer', requireAuth, async (req: Request, res: Response) => {
  try {
    const { batchId } = req.params;
    const transferData: TransferBatchRequest = req.body;
    ...
    const newSupplyChainEntry: SupplyChainEntry = {
...
router.post('/batches/:batchId/environmental-data', async (req: Request, res: Response) => {
  try {
    const { batchId } = req.params;
    ...
    let newAlert: QualityAlert | null = null;
    if (anomalyResult.isAnomaly) {
      newAlert = createQualityAlert(environmentalData, anomalyResult);
      
      if (newAlert) {
        // Add alert to batch
        await database.medicineBatches.updateOne(
          { batchId },
          {
...
router.get('/batches/:batchId/environmental-data', requireAuth, async (req: Request, res: Response) => {
  try {
    const { batchId } = req.params;
    const { page, limit, skip } = getPaginationParams(req.query);
...
router.put('/batches/:batchId/alerts/:alertId/resolve', requireAuth, requireRole('manufacturer', 'admin'), async (req: Request, res: Response) => {
  try {
...
router.get('/statistics', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const response: ApiResponse = {
      ...
      data: {
        totalBatches,
        batchesByStage: (batchesByStage as Array<{ _id: string; count: number }>).reduce(
          (
        ...
        batchesByQuality: (batchesByQuality as Array<{ _id: string; count: number }>).reduce(
          (
            acc: Record<string, number>,
            item: { _id: string; count: number }
          ) => {
