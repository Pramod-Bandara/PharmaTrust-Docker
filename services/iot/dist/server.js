...
import { MLAnomalyDetector } from './ml/anomalyDetector.js';
dotenv.config();
const app = express();
...
function detectAnomalyBasic(temperature, humidity) {
    ...
    let severity = 'LOW';
    if (tempOut && (temperature < TEMP_MIN - 2 || temperature > TEMP_MAX + 2))
        severity = 'HIGH';
    if (humOut && (humidity < HUM_MIN - 5 || humidity > HUM_MAX + 5))
...
app.get('/ml/statistics', async (_req, res) => {
    try {
        if (!mlDetector) {
...
app.get('/ml/batch/:batchId/statistics', async (req, res) => {
    try {
        if (!mlDetector) {
            return res.status(503).json({ error: 'ML detector not initialized' });
        }
