...
export class MLAnomalyDetector {
    ...
    detectSuddenChange(readings, currentReading) {
    ...
    classifySeverity(tempAnomaly, humAnomaly, tempZScore, humZScore, medicineModel, currentReading) {
        const { temperature, humidity } = currentReading;
        const { criticalThresholds } = medicineModel;
        // Critical level - immediate danger
        if (temperature > criticalThresholds.temperature.critical ||
        ...
        if (temperature > criticalThresholds.temperature.danger ||
    ...
    async predictFutureValues(batchId, readings) {
        ...
        const tempRisk = Math.max(0, Math.min(1, Math.abs(nextTemp - medicineModel.temperatureRange.optimal) /
            (medicineModel.temperatureRange.max - medicineModel.temperatureRange.min)));
        const humRisk = Math.max(0, Math.min(1, Math.abs(nextHum - medicineModel.humidityRange.optimal) /
